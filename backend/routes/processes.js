...
processesRoutes.post('/', async (req, res) => {
  try {
    const { name, description, owner_id, due_date, category = 'auditoria', process_month, responsible_ids = [], participant_ids = [], depends_on_id } = req.body;
    const id = uuidv4();
    ...
    await runAsync(
      `INSERT INTO processes (id, name, description, owner_id, due_date, category, depends_on_id, process_month)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [id, name, description, owner_id, safeDueDate, category, depends_on_id || null, process_month || null]
    );
...
processesRoutes.put('/:id', async (req, res) => {
  try {
    ...
    if (responsible_ids && responsible_ids.length > 0) {
      await runAsync('DELETE FROM process_members WHERE process_id = $1', [req.params.id]);
      await runAsync('DELETE FROM process_completion_status WHERE process_id = $1', [req.params.id]);

      for (let member_id of responsible_ids) {
        await runAsync(
          `INSERT INTO process_members (id, process_id, member_id) VALUES ($1, $2, $3)`,
          [uuidv4(), req.params.id, member_id]
        );
